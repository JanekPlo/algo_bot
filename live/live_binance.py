...
def wait_for_next_close(
    exchange: BinanceFuturesAdapter, symbol: str, timeframe: str, last_ts: int
) -> int:
...
def main():
    ...
    ap.add_argument(
        "--tpsl_mode",
        choices=["server", "local", "hybrid"],
        default="local",
        help="server=TP/SL na giełdzie; local=TP/SL lokalnie; hybrid=TP serwerowo, SL lokalnie",
    )
    ap.add_argument(
        "--price_feed",
        choices=["mainnet_mark", "mainnet_last", "testnet_mark", "testnet_last"],
        default="mainnet_mark",
        help="Źródło ceny do lokalnych TP/SL",
    )
    ap.add_argument(
        "--poll_ms", type=int, default=1000, help="Jak często sprawdzać lokalne TP/SL (ms)"
    )
    ap.add_argument(
        "--cat_sl_pct",
        type=float,
        default=0.15,
        help="Awaryjny szeroki SL serwerowy przy tpsl_mode=local",
    )
    ...
    # osobny klient do feedu cen (zeby 'mainnet_mark' byl z mainnetu)
    ...
    try:
        ...
        equity = (bal.get("info", {}) or {}).get("totalMarginBalance") or bal.get("total", {}).get(
            "USDT"
        )
    ...
    while True:
        ...
        try:
            ...
            bal = ex_trade.exchange.fetch_balance()
            equity = (bal.get("info", {}) or {}).get("totalMarginBalance") or bal.get(
                "total", {}
            ).get("USDT")
            wallet = (bal.get("info", {}) or {}).get("totalWalletBalance")
            unreal = (bal.get("info", {}) or {}).get("totalUnrealizedProfit")
        ...
        if (
            ...
            and abs(prev_pos) > 0
            and abs(pos) == 0
            and args.tpsl_mode in ("server", "hybrid")
        ):
            px = last_px or _get_price(args.price_feed)
            e = current["entry_price"]
            side = current["side"]
            qty = current["qty"]
            ...
            if current is None:
                if b_qty > 0 and b_side in ("long", "short"):
                    ...
                    current = {
                        ...
                        "side": b_side,
                        "qty": b_qty,
                        "entry_price": b_entry or (last_px or 0.0),
                    }
                    journal.log_entry(
                        trade_id,
                        args.symbol,
