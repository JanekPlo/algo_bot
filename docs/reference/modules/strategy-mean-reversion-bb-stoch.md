...
| 11 | Pyramiding: one x1 add-on with confirming-candle trigger A or Stoch trigger B [[mms/02](../../references/mms/02-position-management-filters.md)] | Not implemented | **N (deferred)** | Single-position `backtesting.py` cannot express it; separate ADR post-Sweep (Decision 6) |
