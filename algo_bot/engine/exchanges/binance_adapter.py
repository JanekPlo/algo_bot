...
class BinanceFuturesAdapter:
    ...
    def fetch_positions(self, symbol: str) -> float:
        ...
        for p in self.exchange.fetch_positions([symbol]):
            sym_ok = (p.get("symbol") in (symbol, m_id)) or (
                p.get("info", {}).get("symbol") in (symbol, m_id)
            )
            if not sym_ok:
                continue
    ...
    def set_tpsl(
        self, symbol, side, entry_price, tp_pct=None, sl_pct=None, price_type="MARK_PRICE"
    ):
        """
        ...
        """
        working_type = (
            "MARK_PRICE" if (price_type or "").upper() == "MARK_PRICE" else "CONTRACT_PRICE"
        )
        params_base = {
            ...
            try:
                results.append(
                    self.exchange.create_order(
                        ...
                        params={
                            **params_base,
                            "stopPrice": self.price_to_precision(symbol, tp_price),
                        },
                    )
                )
            ...
            try:
                results.append(
                    self.exchange.create_order(
                        ...
                        params={
                            **params_base,
                            "stopPrice": self.price_to_precision(symbol, sl_price),
                        },
                    )
                )
            except Exception as e:
