...
# FIXME (decyzja faza 1): broken — algo_bot.backtester nie istnieje (jest engine.backtester).
# Optimize_backtest tez nie istnieje (logika w engine/sweep.py). Patrz docstring na gorze.
from algo_bot.backtester import optimize_backtest, run_backtest
...
def parse_args():
    ...
    parser.add_argument(
        "--config", "-c", default="config/config.yaml", help="Ścieżka do config.yaml"
    )
...
def main():
    ...
    args = parse_args()
    print(
        f">>> Parsed args: symbol={args.symbol}, timeframe={args.timeframe}, strategy={args.strategy}, optimize={args.optimize}"
    )
